---
title: "DengAI: Predicting Disease Spread, Creating  Neural Network Forecasting Model"
...
I am aiming to build a dynamic regression model.
Accordingly, to select variables as external regressorsm I want to examine the correlations of the feature variables with total cases.
...
and reanalysis_dew_point_temp_k as external regressors.

However, I had trouble in constructing this model as seen in the code below:

### San Juan
```{r}
sj_tsibble<-as_tsibble(sj)
sjdynamicregression <- auto.arima(sj_tsibble, xreg = reanalysis_specific_humidity_g_per_kg, reanalysis_dew_point_temp_k)
```

### Iquitos
```{r}
iq_tsibble<-as_tsibble(iq)
iqdynamicregression <- auto.arima(iq_tsibble, xreg = reanalysis_specific_humidity_g_per_kg, reanalysis_dew_point_temp_k)
```
